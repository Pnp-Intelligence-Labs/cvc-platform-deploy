"""
api/routes/enrichment.py — Data quality enrichment endpoints.

Provides admin interface for reviewing and approving LLM predictions.

Quick Add by URL (POST /quickadd):
  Paste a company website → system scrapes the page, runs LLM enrichment,
  and saves to DB immediately as a background task.
  enrichment_source = 'quickadd', enrichment_status = 'enriching' while running.
"""
...
_QA_VALID_SECTORS = ["Robotics", "Supply Chain", "Industrial Automation", "Physical AI", "Manufacturing", "Other"]
...
- sector: One of: Robotics, Supply Chain, Industrial Automation, Physical AI, Manufacturing, Other
        ...
        with conn.cursor() as cur:
            ...
            if data.run_4d:
                spec = (
                    ...
                    f"Command: cd ~/repos/cvc-intelligence && source ~/.env.secrets && "
                ...
                cur.execute("""
                    ...
                    VALUES (%s, 'medium', 'low', FALSE, 'pending', 'nate', 'bigclaw', 'enrichment', %s, NOW())
                    RETURNING task_id
                """, (spec, data.notes))
            ...
            if data.run_dd:
                ...
                """, (company_id, user["username"], user["username"]))
                dd_task_id = "queued"
...
async def get_dd_status(company_id: int, user=Depends(require_auth)):
    ...
    all_agents_done = all(agent_stages.values())
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                ...
                SET status = 'discovered', status_changed_at = NOW(), changed_by = 'nate',
...
async def trigger_enrich_batch(
    ...
    script_path = repo_root / "scripts" / "enrich_company_data.py"
...
async def get_enrichment_list(
    ...
    # Build set of company_ids with completed DD (filesystem check — only companies
    # with a lifecycle record have a workdir, so this is fast regardless of list size).
...
async def get_dd_routing(company_id: int, user=Depends(require_auth)):
    ...
    If manifest.json exists, groups are by doc_type from the manifest's routing table.
    If not, groups are individual root files.
    Any saved routing_override.json is merged in as the active assignment.
    """
    company_name = _get_company_name(company_id)
    base = DD_WORKDIR / company_name
    AGENTS = ["financials", "comp", "qualitative", "product", "news", "generalist"]
    ...
    # Map filename → doc_type from documents list
    fn_to_type: dict = {d["filename"]: d.get("doc_type", "unknown") for d in documents}

    # Build doc_type → set of agents
    type_to_agents: dict = {}
    ...
    # Build groups sorted by file count descending
    type_counts: dict = {}
...
async def trigger_dd_action(
    ...
    full     — runs run_three.py immediately as a background task (no approval needed)
    research — queues industrial enrichment via SharpClaw, no approval
...
async def approve_suggestion(
    ...
    Optional body: { source_url } to override the worker-supplied source before writing.
    """
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            ...
            # Apply analyst URL override if provided
            if body.source_url:
            ...
            if row["suggestion_type"] == "new_funding_round":
                ...
                if existing:
                    # Round already exists — attach source URL as additional confirmation
                    if source_url:
        ...
        with conn.cursor() as cur:
            cur.execute("""
                SELECT
                    ...
                    COUNT(*) FILTER (
                        WHERE sector IN ('Robotics','Supply Chain','Manufacturing','Industrial Automation','Physical AI')
                    ) as industrial_total,
                    COUNT(*) FILTER (
                        WHERE sector IN ('Robotics','Supply Chain','Manufacturing','Industrial Automation','Physical AI')
...
def get_master_activity_log(
    ...
    Returns entries + total count for pagination.
    """
    conditions = []
    params = []
...
def get_enrichment_step_status(company_id: int, user=Depends(require_auth)):
    ...
    Used by the company profile panel to poll for progress after triggering a step.

    Completion signals:
      founder  — companies.founders is not null/empty, OR company_intel row exists
      fourD    — env_4d is not null
      funding  — funding_rounds count > 0 OR pending new_funding_round intel_suggestions
      deep     — intel_suggestions case_study count > 0 OR case_studies jsonb not empty
    """
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("""
            ...
            if not founder_done:
                # Only count intel rows that actually contain founder data (not the
                # "no founders found" stub that the worker writes on failed discovery)
                cur.execute(
    ...
    def _fmt(ts):
        """Return ISO timestamp as a short human date, e.g. 'Apr 30'."""
        if not ts:
            return None
        try:
...
def _parse_scorecard_feedback(xlsx_bytes: bytes) -> list[dict]:
    """
    Parse the analyst feedback columns from the 'All Findings' sheet.
    Returns a list of finding dicts with accuracy_rating, flag_rating, analyst_notes.
    """
    try:
        import openpyxl
        import io
        ...
        # Find the header row (contains "Agent" and "Accuracy")
        header_row = None
        for i, row in enumerate(rows):
            if row and any(str(c or "").strip().lower() == "agent" for c in row):
                header_row = i
                break
        ...
        i_agent    = col("agent")
        i_topic    = col("topic")
        i_finding  = col("our finding")
        i_claimed  = col("claimed")
        i_verdict  = col("verdict")
        i_flag     = col("flag")
        i_accuracy = col("accuracy")
        i_flagrate = col("flag rating")
        i_notes    = col("notes / correction")
        results = []
        for row in rows[header_row + 1:]:
            ...
            accuracy = get(i_accuracy)
            flag_r   = get(i_flagrate)
            notes    = get(i_notes)
            if not any([accuracy, flag_r, notes]):
                continue
            results.append({
                "agent":           get(i_agent).lower(),
                "topic":           get(i_topic),
                "our_finding":     get(i_finding),
                "claimed":         get(i_claimed),
                "verdict":         get(i_verdict),
                "was_flagged":     get(i_flag).upper() == "YES",
        ...
        _dd_log.error(f"Scorecard parse error: {e}")
        return []


def _store_feedback(company_id: int, company_name: str, findings: list[dict], reviewed_by: str):
    """Upsert feedback rows — delete old rows for this company first, then insert fresh."""
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("DELETE FROM cvc.dd_feedback WHERE company_name = %s", (company_name,))
            for f in findings:
                cur.execute("""
                    INSERT INTO cvc.dd_feedback
                        (company_id, company_name, agent, topic, our_finding, claimed,
                         verdict, was_flagged, accuracy_rating, flag_rating, analyst_notes, reviewed_by)
                    VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
                """, (
                    company_id, company_name,
                    f.get("agent"), f.get("topic"), f.get("our_finding"), f.get("claimed"),
                    f.get("verdict"), f.get("was_flagged"),
                    f.get("accuracy_rating"), f.get("flag_rating"), f.get("analyst_notes"),
                    reviewed_by,
                ))
        conn.commit()


def _update_learning_summary():
    """
    Aggregate dd_feedback into a learning summary and write to platform_settings.
    DD agents pick this up on next run via ConfigLoader.
    """
    try:
        with get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT agent, topic, verdict,
                           COUNT(*) FILTER (WHERE flag_rating = 'over-flagged')        AS over_flagged,
                           COUNT(*) FILTER (WHERE flag_rating = 'should have been flagged') AS missed,
                           COUNT(*) FILTER (WHERE accuracy_rating = 'wrong')           AS wrong,
                           COUNT(*) FILTER (WHERE accuracy_rating = 'correct')         AS correct,
                           COUNT(*) AS total
                    FROM cvc.dd_feedback
                    WHERE flag_rating IS NOT NULL OR accuracy_rating IS NOT NULL
                    GROUP BY agent, topic, verdict
                    ORDER BY (COUNT(*) FILTER (WHERE flag_rating = 'over-flagged') +
                              COUNT(*) FILTER (WHERE flag_rating = 'should have been flagged')) DESC
                    LIMIT 50
                """)
            ...
            summary = {
                ...
                "patterns": patterns,
                "guidance": (
                    "These are patterns from analyst-reviewed DD runs. "
                    "When flagging, consider that topics with high over_flagged counts are frequently "
                    "false positives. Topics with high missed counts are under-detected."
                ),
            }

            with conn.cursor() as cur:
                cur.execute("""
                    INSERT INTO cvc.platform_settings (key, value, updated_at)
                    VALUES ('dd_agent_learning', %s, NOW())
                    ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = NOW()
                """, (json.dumps(summary),))
            conn.commit()
    except Exception as e:
        _dd_log.error(f"Learning summary update failed: {e}")


def _run_review_agent(company_name: str):
    """Background task: run reviewer agent then render PDF."""
    import subprocess
    import os
    _REPO_ROOT = Path(__file__).resolve().parents[2]
    _DD_DIR    = _REPO_ROOT / "workers" / "dd"
    env = {**os.environ, "PYTHONPATH": str(_REPO_ROOT / "core")}
    try:
        subprocess.run(
            ["python3", "-m", "agents.reviewer.agent", company_name],
            cwd=_DD_DIR, env=env, timeout=1800, check=True,
        )
    except Exception as e:
        _dd_log.error(f"Review agent failed for {company_name}: {e}")


@router.post("/dd/{company_id}/submit-review")
async def submit_dd_review(
    company_id: int,
    background_tasks: BackgroundTasks,
    scorecard: UploadFile = File(...),
    appendix: UploadFile = File(None),
    user=Depends(require_auth),
):
    """
    Accept analyst-annotated Scorecard XLSX (and optionally Appendix PDF).
    Parses feedback columns, stores in dd_feedback, saves files to workdir,
    then fires the reviewer agent in the background.
    """
    company_name = _get_company_name(company_id)
    workdir = DD_WORKDIR / company_name
    if not workdir.exists():
        raise HTTPException(status_code=404, detail="DD workdir not found — run pipeline first")

    # Save uploaded scorecard
    sc_bytes = await scorecard.read()
    if not sc_bytes:
        raise HTTPException(status_code=400, detail="Scorecard file is empty")
    safe = company_name.replace(" ", "_").replace("/", "-")
    reviewed_path = workdir / f"{safe}_Scorecard_Reviewed.xlsx"
    reviewed_path.write_bytes(sc_bytes)

    # Save appendix if provided
    if appendix and appendix.filename:
        apx_bytes = await appendix.read()
        if apx_bytes:
            (workdir / f"{safe}_Appendix_Reviewed.pdf").write_bytes(apx_bytes)

    # Parse feedback
    findings = _parse_scorecard_feedback(sc_bytes)
    _dd_log.info(f"[review] {company_name}: {len(findings)} annotated findings parsed")

    # Store in DB
    _store_feedback(company_id, company_name, findings, user["username"])

    # Update learning summary
    _update_learning_summary()

    # Fire reviewer agent
    background_tasks.add_task(_run_review_agent, company_name)

    return {
        "status":          "review_started",
        "company":         company_name,
        "findings_parsed": len(findings),
        "message":         f"Parsed {len(findings)} annotated findings. Review memo generating in background.",
    }


@router.get("/dd/{company_id}/review-status")
async def get_review_status(company_id: int, user=Depends(require_auth)):
    """Check whether a review memo exists for this company."""
    company_name = _get_company_name(company_id)
    safe = company_name.replace(" ", "_").replace("/", "-")
    workdir = DD_WORKDIR / company_name
    has_review_memo = (workdir / "review_memo.json").exists()
    has_review_pdf  = (workdir / f"{safe}_Review_Memo.pdf").exists()
    has_review_docx = (workdir / f"{safe}_Review_Memo.docx").exists()
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT COUNT(*), MAX(reviewed_at) FROM cvc.dd_feedback WHERE company_name = %s",
                (company_name,)
            )
            row = cur.fetchone()
            feedback_count = row[0] if row else 0
            last_reviewed  = row[1].isoformat() if row and row[1] else None
    return {
        "company":         company_name,
        "has_review_memo": has_review_memo,
        "has_review_pdf":  has_review_pdf,
        "has_review_docx": has_review_docx,
        "feedback_count":  feedback_count,
        "last_reviewed":   last_reviewed,
    }


# ── Signal Queue — manual YouTube ingestion ──────────────────────────────────
...
def _fetch_youtube_title(video_id: str) -> Optional[str]:
    """Fetch real video title via YouTube oEmbed (no API key, not IP-blocked)."""
    try:
...
def ingest_youtube(payload: YoutubeIngestRequest, user=Depends(require_auth)):
    """Fetch YouTube captions and queue the episode for signal enrichment."""
    video_id = _extract_video_id(payload.url)
    if not video_id:
        raise HTTPException(status_code=400, detail="Could not extract video ID from URL")

    content_hash = f"yt-{video_id}"
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            # Dedup check
            cur.execute(
                "SELECT id, title, enrichment_status FROM cvc.content_items WHERE content_hash = %s",
                (content_hash,),
            )
            existing = cur.fetchone()
            if existing:
                return {
                    "status": "duplicate",
                    "message": "This episode is already in the signal queue.",
                    "item": {"id": existing["id"], "title": existing["title"], "enrichment_status": existing["enrichment_status"]},
                }

    # Fetch captions outside the DB connection (network call)
    transcript = None
    ...
    try:
        ...
        pass  # No captions — still insert, enrichment worker can skip or we note it

    if not transcript or len(transcript) < 100:
        # Insert as stub — no transcript available from captions
        transcript = ""
        transcript_source = "none"

    title = (payload.title or "").strip() or _fetch_youtube_title(video_id) or f"YouTube Episode ({video_id})"
    import json as _json
    tags = _json.dumps(["podcast", "manual", transcript_source])

    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                INSERT INTO cvc.content_items
                    (content_type, title, url, raw_text, summary, tags, enrichment_status, content_hash)
                VALUES ('podcast_episode', %s, %s, %s, %s, %s::jsonb, 'raw', %s)
                RETURNING id, title, enrichment_status, created_at
            """, (
                title,
                canonical_url,
                transcript[:50000],
    ...
    return {
        "status": "queued",
        "message": f"Queued. Transcript will be fetched by Refinery on next collection run (4:30 AM UTC).",
        "item": {
            "id":                 row["id"],
            "title":              row["title"],
...
def list_signal_queue(user=Depends(require_auth)):
    """List manually queued YouTube episodes."""
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT id, title, url, enrichment_status, tags, created_at,
                       LENGTH(raw_text) AS transcript_chars
...
@router.get("/intro-matches")
def list_intro_matches(user=Depends(require_auth)):
    """Return all unreviewed suggested company matches for partner intro rows."""
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT
                    pi.startup_name,
                    pi.source,
                    pi.suggested_company_id,
                    c.name AS suggested_company_name,
                    pi.match_confidence,
                    COUNT(*) AS intro_count
                FROM cvc.partner_intros pi
                JOIN cvc.companies c ON c.id = pi.suggested_company_id
                WHERE pi.suggested_company_id IS NOT NULL
                  AND pi.company_id IS NULL
                  AND pi.match_reviewed = FALSE
                GROUP BY pi.startup_name, pi.source, pi.suggested_company_id,
                         c.name, pi.match_confidence
                ORDER BY pi.match_confidence DESC, pi.startup_name
            """)
            rows = cur.fetchall()
    return {"matches": [
        {
            ...
            "intro_count":            int(r["intro_count"]),
        }
        for r in rows
    ]}


@router.post("/intro-matches/{startup_name}/confirm")
def confirm_intro_match(startup_name: str, user=Depends(require_auth)):
    """Confirm the suggested match: set company_id = suggested_company_id."""
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                UPDATE cvc.partner_intros
                SET company_id = suggested_company_id,
                    match_reviewed = TRUE
                WHERE startup_name = %s
                  AND suggested_company_id IS NOT NULL
...
@router.post("/intro-matches/{startup_name}/reject")
def reject_intro_match(startup_name: str, user=Depends(require_auth)):
    """Reject the suggested match: clear suggested_company_id, mark reviewed."""
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                UPDATE cvc.partner_intros
                SET suggested_company_id = NULL,
