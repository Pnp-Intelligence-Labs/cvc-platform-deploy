...
- `docs/PLUGIN_INTERFACE.md`: corrected the "`plugins/installed/` is in `.gitignore` / never committed" claim — it is now tracked so managed deploys ship the bundled plugins.
- `CLAUDE.md`: added a **Managed Deploy (live)** section (Vercel `verticalos` → pnpverticalos.vercel.app; Railway `cvc-api`; Supabase `DATABASE_URL`; note that the backend is NOT deployed on Vercel).
- **Diagnosed the `vercel-runtime==0.14.1` Python build failure:** a second Vercel project `cvc-platform-deploy` (created 4h ago, root dir `.`, Framework Preset **FastAPI**) was auto-importing the repo and trying to build the FastAPI backend on Vercel serverless — fails because `vercel-runtime` needs Python ≥3.12 (Vercel env is 3.11). The real frontend project `verticalos` (root `designs/figma-dashboard`) builds fine. The FastAPI-on-Vercel project is architecturally wrong (backend lives on Railway) and only produces failing builds. **Resolved:** deleted the `cvc-platform-deploy` Vercel project (`vercel project rm`, user-approved); only `verticalos` remains. Push-triggered build failures stop.
...
- **CI (Security workflow) was red on main:** `uv lock --check` — upstream Dependabot pip merges (uvicorn 0.49, pdfplumber 0.11.9, django-cors-headers, google-auth-httplib2) landed without re-locking; my rebase re-locked, latest push carries the fresh lock.
...
## 2026-06-11 — Drive connect 403 diagnosed + Drive OAuth/ingest unit tests
- **Drive "Access blocked / Error 403: access_denied" diagnosed:** not a code bug. The Google OAuth consent screen for the app's client (`962909194589-...`) is in **Testing** mode, and `harshal.pathak3@gmail.com` is not on the test-user list, so Google refuses consent before our callback ever runs. Fix is in Google Cloud Console → APIs & Services → OAuth consent screen → Audience → **Test users → add the Gmail/Workspace accounts** that need Drive connect (up to 100). Publishing to Production instead would trigger Google's restricted-scope verification (we request full `auth/drive`), so Testing + test users is the right mode for now.
- **Admin account unchanged:** `admin` / `changeme` password login (`POST /auth/login`) is independent of Google SSO — Google login/auto-provision is an additional path, not a replacement. No code change needed; confirmed per user request.
...
- Shipped in e40de8a together with the RoutedDocs workstream (`/terminal/routed` + per-tab "From your Terminal" panels). Frontend live via Vercel auto-deploy; backend deployed to Railway (`railway up`, verified `/terminal/ingest-link` in live OpenAPI).
